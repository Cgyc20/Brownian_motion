...
ax.plot(true_path[:, 0], true_path[:, 1], marker='o', markersize=3, linewidth=1, label='True trajectory', color='blue')
#Extract the inferred means from the algorithm
ax.plot(inferred_path[:, 0], inferred_path[:, 1], marker='x', markersize=3, linewidth=1, label='Inferred trajectory', color='red')
#Formatting
ax.set_xlim(box[0])
ax.set_ylim(box[1])
